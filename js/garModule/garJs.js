...
!!(function () {
    ...
    for (; len--;) {
        ...
        gar.addHandler(oInput, 'keyup', function (e) {
            transform(this, e);
        }, false);
    }
    ...
    ProgressBar.fn.dynamicMsg = function (_self, i, per, _aI, times) {
        ...
        _self.spanDetail.innerHTML = (function (per) {
            switch (true) {
                ...
                case per >= 0.2 && per < 0.4:
                    return 'SEARCHING FOR YOUR INFO FROM DATABASE!';
                case per >= 0.4 && per < 0.6:
                    return 'HANDLING RELATED CALCULATION IN THE SERVER!';
                case per >= 0.6 && per < 0.8:
                    return 'GOT INFO AND READY TO SEND FROM THE SERVER!';
                case per >= 0.8 && per < 1.0:
